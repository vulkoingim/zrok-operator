...
4. **Share ownership is three-way.** Inventory remote `ListShares` + agent Status + CR. Adopt / Unshare only if **target matches** `spec.upstream`. Reserved name held by a different target → `Ready=False` reason `NameConflict`, **do not Unshare**. Ours remotely + agent empty (registry wipe) → Unshare **our** token then SharePublic.
